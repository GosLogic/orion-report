...
## 4.3.2 Iteration 2: Refining Telemetry Persistence and Offline-First Operation

En la segunda iteración del Attribute-Driven Design (ADD v3), el objetivo es refinar los elementos arquitectónicos que quedaron parcialmente abordados en la Iteración 1. En particular, esta iteración se enfoca en fortalecer la persistencia de telemetría mediante TimescaleDB, asegurar la continuidad operativa de la aplicación móvil en zonas sin conectividad y mejorar la resiliencia frente a fallas o sobrecostos derivados del uso de Google Maps.

A diferencia de la primera iteración, donde se estableció la estructura inicial del sistema, esta segunda iteración profundiza en componentes específicos: MobileApp, TelemetryMapsService, TelemetryDB, MessageBroker y Redis Cache. Estos elementos son críticos para cumplir con los atributos de calidad de performance, disponibilidad y usabilidad definidos previamente.

### 4.3.2.1 Architectural Design Backlog 2

El Architectural Design Backlog de esta iteración agrupa los drivers que requieren refinamiento específico luego de haber definido la topología general de Orion. Estos drivers están vinculados a la operación en campo, la ingesta histórica de telemetría y la dependencia de servicios externos.

| Driver ID | Tipo de driver | Descripción del driver |
|----------|----------------|-------------------------|
| US-08 | Primary functionality | Visualización de vehículos en tiempo real sobre mapa para optimizar la logística. |
| US-09 | Primary functionality | Consulta histórica de rutas y procesamiento de telemetría vehicular. |
| US-21 | Primary functionality | Persistencia local offline de eventos en SQLite desde la aplicación móvil. |
| US-22 | Primary functionality | Sincronización automática de eventos pendientes al recuperar conectividad. |
| QA-01 | Quality attribute | Performance: mantener latencia p95 menor o igual a 3 segundos bajo carga nominal de telemetría. |
| QA-02 | Quality attribute | Disponibilidad: activar modo degradado ante fallas de Google Maps en menos de 2 segundos. |
| QA-03 | Quality attribute | Usabilidad: sincronizar al menos el 95% de eventos offline en menos de 60 segundos y sin duplicados. |
| CON-01 | Constraint | Uso obligatorio de Google Maps como proveedor externo de visualización cartográfica. |
| CON-02 | Constraint | Operación en zonas con conectividad móvil intermitente o nula. |
| CON-05 | Constraint | Uso de tecnologías Open Source para persistencia, caché y mensajería. |
| CRN-02 | Concern | Riesgo de dependencia crítica y sobrecostos por consumo intensivo de Google Maps. |
| CRN-03 | Concern | Riesgo de sobrecarga por ingesta masiva concurrente de coordenadas GPS. |
| CRN-04 | Concern | Riesgo de pérdida de eventos cuando el conductor opera sin conexión. |

### 4.3.2.2 Establish Iteration Goal by Selecting Drivers

El objetivo principal de esta iteración es refinar la arquitectura operativa de Orion para garantizar continuidad, resiliencia y rendimiento en escenarios reales de uso. Para ello, se seleccionan drivers relacionados con tres problemas críticos: la telemetría masiva, la conectividad intermitente y la dependencia de Google Maps.

Primero, se aborda la necesidad de optimizar TelemetryDB, ya que el sistema debe almacenar grandes volúmenes de coordenadas GPS sin afectar la consulta histórica de rutas ni la visualización en tiempo real. Esto permite responder al escenario de performance relacionado con la latencia de telemetría.

Segundo, se refina el diseño offline-first de la MobileApp. Dado que los conductores transitan por zonas con baja o nula cobertura, el sistema debe permitir registrar eventos, incidentes y jornadas incluso sin conexión, almacenándolos localmente y sincronizándolos de forma automática cuando la red se recupere.

Tercero, se aborda la dependencia de Google Maps mediante mecanismos de resiliencia y optimización de costos. Para evitar fallos en cascada, el TelemetryMapsService deberá incorporar Circuit Breaker, caché con Redis y una estrategia de modo degradado que permita mantener la operación básica aunque el proveedor externo falle.

### 4.3.2.3 Choose One or More Elements of the System to Refine

En esta iteración no se refina el sistema completo, sino un conjunto específico de elementos arquitectónicos que ya fueron instanciados en la Iteración 1. Los elementos seleccionados son:

| Elemento a refinar | Razón de refinamiento |
|--------------------|------------------------|
| MobileApp | Debe soportar operación offline, persistencia local en SQLite y sincronización automática. |
| TelemetryMapsService | Debe procesar coordenadas, consultar mapas y aplicar Circuit Breaker frente a fallas de Google Maps. |
| TelemetryDB | Debe optimizarse como base de datos de series de tiempo para almacenar grandes volúmenes de telemetría. |
| MessageBroker | Debe asegurar el desacoplamiento entre la ingesta de coordenadas y los servicios consumidores. |
| Redis Cache | Debe reducir consultas repetidas a Google Maps y mejorar tiempos de respuesta ante solicitudes frecuentes. |

Estos elementos son seleccionados porque concentran los riesgos técnicos más importantes de la operación real de Orion: pérdida de datos en campo, degradación por alta carga y dependencia de proveedores externos.

### 4.3.2.4 Choose One or More Design Concepts That Satisfy the Selected Drivers

Para satisfacer los drivers seleccionados, se aplican los siguientes conceptos de diseño:

| Concepto de diseño | Categoría | Drivers satisfechos | Justificación |
|--------------------|-----------|---------------------|---------------|
| Offline-First Architecture | Arquitectura de cliente móvil | US-21, US-22, QA-03, CON-02, CRN-04 | Permite que el conductor registre eventos sin conexión, almacenándolos en SQLite hasta que la red se recupere. |
| Local Outbox Pattern | Patrón de persistencia móvil | US-21, US-22, QA-03 | Mantiene una cola local de eventos pendientes, evitando pérdida de información y permitiendo sincronización controlada. |
| Retry with Exponential Backoff | Táctica de resiliencia | US-22, QA-03, CRN-04 | Permite reintentar envíos fallidos sin saturar el backend ni duplicar eventos. |
| Idempotency Keys | Táctica de confiabilidad | US-22, QA-03 | Evita la duplicidad de eventos cuando la app reintenta sincronizaciones después de fallos de red. |
| TimescaleDB Hypertables | Tecnología de persistencia | US-09, QA-01, CRN-03 | Optimiza el almacenamiento de coordenadas GPS como series de tiempo, facilitando consultas históricas por unidad, tenant y rango de fechas. |
| Cache-Aside Pattern with Redis | Patrón de caché | US-08, QA-02, CON-01, CRN-02 | Reduce llamadas repetidas a Google Maps y disminuye latencia en consultas cartográficas frecuentes. |
| Circuit Breaker | Táctica de disponibilidad | US-08, QA-02, CON-01, CRN-02 | Evita que errores sostenidos de Google Maps provoquen fallas en cascada dentro del sistema. |
| Dead Letter Queue | Táctica de confiabilidad | QA-01, CRN-03 | Permite aislar eventos de telemetría que no puedan procesarse correctamente sin bloquear el flujo principal. |

### 4.3.2.5 Instantiate Architectural Elements, Allocate Responsibilities, and Define Interfaces

A partir de los conceptos seleccionados, se instancian nuevos componentes internos dentro de los contenedores ya definidos en la arquitectura de Orion.

| Elemento instanciado | Contenedor asociado | Responsabilidad asignada | Interfaz preliminar |
|----------------------|--------------------|---------------------------|---------------------|
| LocalEventStore | MobileApp | Almacenar eventos offline de jornada, incidentes, coordenadas y confirmaciones en SQLite. | SQLite local |
| SyncEngine | MobileApp | Detectar conectividad disponible, leer eventos pendientes y enviarlos al backend en segundo plano. | HTTPS/REST |
| ConnectivityMonitor | MobileApp | Verificar cambios de red y activar la sincronización automática. | API nativa del dispositivo |
| OutboxQueue | MobileApp | Mantener la cola local de eventos pendientes con estado: pending, syncing, synced o failed. | SQLite local |
| MobileSyncEndpoint | ApiGateway / Backend | Recibir lotes de eventos sincronizados desde la app móvil y validar token, TenantId e idempotency key. | HTTPS/REST JSON |
| TelemetryConsumer | TelemetryMapsService | Consumir eventos GPS desde el MessageBroker y persistirlos en TelemetryDB. | AMQP |
| TelemetryRepository | TelemetryMapsService | Encapsular consultas e inserciones sobre TimescaleDB. | TCP/PostgreSQL |
| TelemetryHypertable | TelemetryDB | Almacenar coordenadas GPS particionadas por tiempo, tenant y vehículo. | TimescaleDB |
| MapProviderAdapter | TelemetryMapsService | Aislar la integración con Google Maps mediante una capa de adaptación. | HTTPS/REST |
| CircuitBreakerPolicy | TelemetryMapsService | Bloquear temporalmente llamadas a Google Maps ante errores repetidos. | Política interna |
| MapCache | Redis Cache | Almacenar respuestas frecuentes de geocodificación y datos cartográficos. | Redis/TCP |
| DeadLetterQueue | MessageBroker | Retener eventos de telemetría inválidos o no procesables para revisión posterior. | AMQP |

Con esta asignación, la MobileApp asume la responsabilidad de continuidad operativa en campo, mientras que TelemetryMapsService mantiene la responsabilidad de procesamiento, consulta histórica, integración cartográfica y resiliencia frente a proveedores externos.

### 4.3.2.6 Sketch Views (C4 & UML) and Record Design Decisions

Para esta iteración se incorporan las siguientes vistas:

1. **Diagrama de componentes de MobileApp offline-first**  
    <p>
  <img src="assets/chapter-iv/c4-I/MobileAppOfflineFirstComponents-dark.png"
    alt="NotificationService Component Diagram"
    style="width: 90%; max-width: 900px; height: auto; display: block; margin: 0 auto;"/>
  <br/>
  <em></em>
...
2. **Diagrama de secuencia de sincronización offline**  
   <p>
  <img src="assets/chapter-iv/c4-I/OfflineSynchronizationSequence-dark.png"
    alt="NotificationService Component Diagram"
    style="width: 90%; max-width: 900px; height: auto; display: block; margin: 0 auto;"/>
  <br/>
  <em></em>
...
3. **Diagrama de componentes de TelemetryMapsService**  
   
   <p>
  <img src="assets/chapter-iv/c4-I/TelemetryServiceComponents-dark (2).png"
    alt="NotificationService Component Diagram"
    style="width: 90%; max-width: 900px; height: auto; display: block; margin: 0 auto;"/>
  <br/>
  <em></em>
...
|----|--------------------|---------------|
| ADR-02-01 | Implementar SQLite como almacenamiento local de la MobileApp. | Permite registrar eventos sin conexión y cumple con la restricción de conectividad intermitente. |
| ADR-02-02 | Aplicar Local Outbox Pattern para eventos offline. | Evita pérdida de datos y mantiene trazabilidad de eventos pendientes. |
| ADR-02-03 | Usar idempotency keys en cada evento sincronizado. | Previene duplicados cuando la app reintenta el envío de eventos. |
| ADR-02-04 | Refinar TelemetryDB con hypertables de TimescaleDB. | Optimiza la escritura y consulta de coordenadas GPS de alto volumen. |
| ADR-02-05 | Aplicar Redis Cache para respuestas cartográficas frecuentes. | Reduce costos y latencia asociados al consumo de Google Maps. |
| ADR-02-06 | Implementar Circuit Breaker en TelemetryMapsService. | Protege al sistema frente a fallas sostenidas del proveedor externo. |
| ADR-02-07 | Incorporar Dead Letter Queue para eventos de telemetría fallidos. | Evita que eventos inválidos bloqueen el flujo principal de procesamiento. |

### 4.3.2.7 Analysis of Current Design and Review Iteration Goal (Kanban Board)

En el Paso 7 de ADD v3 hemos evaluado el diseño actual frente a los drivers seleccionados en el Architectural Design Backlog de la segunda iteración, clasificando cada preocupación abordada de manera completa, parcial o aún no resuelta. La tabla siguiente resume el estado de esta revisión y vincula cada agrupación de drivers con las decisiones arquitectónicas tomadas durante el ciclo:

| Driver ID | Estado (Status) | Decisiones de Diseño Tomadas (Design Decisions) |
|----------|-----------------|--------------------------------------------------|
| US-21, US-22, QA-03, CON-02, CRN-04 <br> (Operación offline-first en campo) | Completely Addressed | Se diseñó la arquitectura offline-first de la MobileApp mediante LocalEventStore, OutboxQueue, ConnectivityMonitor, SyncEngine y persistencia local en SQLite. Esta decisión permite registrar eventos sin conectividad y sincronizarlos automáticamente al recuperar la red. |
| US-08, QA-02, CON-01, CRN-02 <br> (Resiliencia frente a Google Maps) | Completely Addressed | Se refinó el TelemetryMapsService incorporando MapIntegrationAdapter, CircuitBreakerPolicy y MapCache con Redis. Esto reduce la dependencia directa del proveedor externo, permite operar en modo degradado y disminuye llamadas repetidas a Google Maps. |
| US-09, QA-01, CRN-03 <br> (Performance e ingesta masiva de telemetría) | Partially Addressed | Se refinó la persistencia histórica mediante TelemetryRepository y TelemetryDB con TimescaleDB Hypertables. Sin embargo, la validación final de latencia p95, throughput e ingesta concurrente deberá comprobarse mediante pruebas de carga en la etapa de implementación. |
| CON-05 <br> (Uso de tecnologías Open Source) | Completely Addressed | Las decisiones de diseño mantienen el uso de tecnologías Open Source como SQLite, TimescaleDB, Redis y Apache ActiveMQ, alineándose con las restricciones tecnológicas y presupuestales del proyecto. |

#### Revisión del Objetivo y Kanban Board

Hemos constatado que el objetivo de esta segunda iteración se cumple satisfactoriamente a nivel de diseño arquitectónico. La arquitectura de Orion ahora cuenta con un refinamiento claro para soportar operación offline en campo, almacenamiento eficiente de telemetría y tolerancia ante fallas del proveedor cartográfico.

Asimismo, se han incorporado vistas complementarias que permiten evidenciar las decisiones tomadas durante la iteración. El diagrama refinado de TelemetryService / TelemetryMapsService muestra la integración de Redis Cache, Circuit Breaker, DeadLetterQueue y TimescaleDB para mejorar la resiliencia y el procesamiento de telemetría. Por otro lado, el diagrama de componentes de MobileApp offline-first y el diagrama de secuencia de sincronización offline explican cómo la aplicación móvil guarda eventos localmente en SQLite y los sincroniza automáticamente al recuperar conectividad.

Para preservar la trazabilidad entre el modelo arquitectónico y la ejecución operativa, hemos actualizado el tablero ágil del equipo con las historias de usuario asociadas, los refinamientos pendientes y las tareas técnicas derivadas de esta iteración. De este modo, el seguimiento de las decisiones de diseño y futuras validaciones se gestiona de forma transparente.

Enlace de Trello:  
[Trello Kanban Board Iteration 2](https://trello.com/invite/b/6a02dd7f6841aff9950f19dc/ATTIddb9625e9b0496ca72f93aec026d46589739B5B6/architectural-design-backlog-2-goslogic)